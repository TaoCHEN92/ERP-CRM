...
namespace erp.dal
{
    public class daocommand : dao
    {
        ...
        string date_done, string date_delivery, string date_pay, string remark, string Is_Done, string Is_Sent, string Is_Paid)
        {
            ...
            var sqlQuery_ = string.Format("UPDATE command SET date_done = NULL , date_delivery = NULL, date_pay = NULL where id_command = '{0}'", id_command);
            ExecuteNonQuery(sqlQuery, variables.SqlConStrERP);
        ...
        string date_done, string date_delivery, string date_pay, string remark, string Is_Done, string Is_Sent, string Is_Paid)
        {
            ...
            if (Is_Paid == "False")
                date_pay = "";
            var sqlQuery = string.Format("UPDATE command SET  id_command_last='{1}',name_product='{2}',date_pre_done='{3}',date_begin='{4}',format='{5}',quantity='{6}',price_unit='{7}',unit='{8}',date_done='{9}',date_delivery='{10}',date_pay='{11}',remark='{12}',Is_Done='{13}',Is_Sent='{14}',Is_Paid='{15}' WHERE id_command='{0}'",
